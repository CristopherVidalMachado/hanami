        ...
        class Action < Command
          ...
          def call(app:, action:, **options)
            *controller, action        = controller_and_action_name(action)
            ...
            context                    = context.with(action_class_content: action_class_content(context))
            context                    = context.with(view_class_content: view_class_content(context))
          ...
          def beginning_module_definition(context)
            ...
            context.classified_controller_name.split("::").each_with_index do |module_name, index|
              beginning_module_definition += "#{indentation}" + "module #{module_name}"
          ...
          def ending_module_definition(context)
            ...
            context.classified_controller_name.split("::").each_with_index do |_, index|
              ending_module_definition += indentation + "end"
              ending_module_definition += "\n" unless length == index + 1
          ...
          def action_class_content(context)
            ...
            add_indentation_to_class_content(context, view_class)
          end

          def view_class_content(context)
            view_class = [
              "class #{context.action.classify}",
          ...
          def add_indentation_to_class_content(context, view_content)
            length = context.classified_controller_name.split("::").size
            first_indentation = " " * 4
